...
### Fill in missing weekly rates for the 2020-2021 flu season ###
# for weeks 40 - 52 in 2020, fill in with weeks 40 - 52 in 2019
# for weeks 1 - 17 in 2021, fill in with weeks 1 - 17 in week 2020
cdc_2019_2020 = cdc2 %>%
  filter(MMWR.YEAR == "2019" | MMWR.YEAR == "2020")
# filling in pediatric
cdc2$WEEKLY.RATE.PEDIATRIC <- ifelse(is.na(cdc2$WEEKLY.RATE.PEDIATRIC),
                                     cdc_2019_2020$WEEKLY.RATE.PEDIATRIC[match(cdc2$MMWR.WEEK, 
                                                                               cdc_2019_2020$MMWR.WEEK)], 
                                     cdc2$WEEKLY.RATE.PEDIATRIC)

# filling in adult
cdc2$WEEKLY.RATE.ADULT <- ifelse(is.na(cdc2$WEEKLY.RATE.ADULT),
                                     cdc_2019_2020$WEEKLY.RATE.ADULT[match(cdc2$MMWR.WEEK, 
                                                                               cdc_2019_2020$MMWR.WEEK)], 
                                     cdc2$WEEKLY.RATE.ADULT)


# checking that 2020-2021 filled in with 2019 - 2020 data for adult and pediatric
plot(cdc2$date, cdc2$WEEKLY.RATE.PEDIATRIC)
plot(cdc2$date, cdc2$WEEKLY.RATE.ADULT)
...
# create week column
hhs2$week = week(hhs2$date)

# arrange by date
hhs2 = hhs2 %>% arrange(date)

# checking for NaN
any(is.nan(hhs2$previous_day_admission_influenza_confirmed_7_day_sum)) # FALSE
...
p = ggplot(hhs2, aes(fill=hospital_subtype, y=previous_day_admission_influenza_confirmed_7_day_sum, x=year)) + 
  geom_bar(position="stack", stat="identity") 
p

plot(hhs2$date, hhs2$previous_day_admission_influenza_confirmed_7_day_sum)


#############################################################
### FILLING MISSING HHS VALUES
############################################################

# fill in missing facility data
# step 1: fill in NAs by median values of same hospital_subtype
# Start by filling in missing values by taking the week median/mean for hospital_subtype

# group by year, week, hospital_subtype
# get median
hhs3 = hhs2 %>% 
  group_by(hospital_subtype, year, week) %>% 
  mutate(flu_median = median(previous_day_admission_influenza_confirmed_7_day_sum, na.rm = TRUE), 
         covid_pediatric_median = median(previous_day_admission_pediatric_covid_confirmed_7_day_sum, na.rm = TRUE), 
         covid_adult_median = median(previous_day_admission_adult_covid_confirmed_7_day_sum, na.rm = TRUE)) %>%
  mutate(flu_na = previous_day_admission_influenza_confirmed_7_day_sum, 
         covid_pediatric_na = previous_day_admission_pediatric_covid_confirmed_7_day_sum, 
         covid_adult_na = previous_day_admission_adult_covid_confirmed_7_day_sum) %>%
  mutate(flu_weekly_new = ifelse(is.na(previous_day_admission_influenza_confirmed_7_day_sum), flu_median, previous_day_admission_influenza_confirmed_7_day_sum), 
         covid_weekly_new_pediatric = ifelse(is.na(previous_day_admission_pediatric_covid_confirmed_7_day_sum), covid_pediatric_median, previous_day_admission_pediatric_covid_confirmed_7_day_sum), 
         covid_weekly_new_adult = ifelse(is.na(previous_day_admission_adult_covid_confirmed_7_day_sum), covid_adult_median, previous_day_admission_adult_covid_confirmed_7_day_sum))

# double checking that ~1000 NAs are filled in by subtype median/week
test = hhs3 %>% 
  select(hospital_subtype, year, week, date, previous_day_admission_influenza_confirmed_7_day_sum, flu_na, flu_weekly_new, flu_median, flu_na, 
         previous_day_admission_pediatric_covid_confirmed_7_day_sum, covid_pediatric_na, covid_weekly_new_pediatric, covid_pediatric_median) %>%
  filter(is.na(flu_na))

# how many missing values are there after filling in weekly median by hospital_subtype?
summary(hhs3$flu_weekly_new) # 3157 NAs
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
# 0.0000  0.0000  0.0000  0.8102  0.0000 91.0000    3157 
summary(hhs3$covid_weekly_new_pediatric) # 933 NAs
summary(hhs3$covid_weekly_new_adult) # 875 NAs


# step 2: forward fill remaining missing values by hospital facility
hhs3 = hhs3 %>%
  dplyr::group_by(hospital_name) %>%
  fill(flu_weekly_new, .direction = "downup") %>%
  fill(covid_weekly_new_pediatric, .direction = "downup") %>% 
  fill(covid_weekly_new_adult, .direction = "downup") %>%
  mutate(year = year) %>%
  dplyr::ungroup()


# how many missing values are there after forward filling in weekly mean by hospital_name?
summary(hhs3$flu_weekly_new) # 0 NAs
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#0.0000  0.0000  0.0000  0.7048  0.0000 91.0000 
summary(hhs3$covid_weekly_new_pediatric) # 0 NAs
summary(hhs3$covid_weekly_new_adult) # 0 NAs

## at this point, all missing HHS flu and covid data should be filled in

####################################################################################################
### MA State flu, covid rates for all hosp subtypes except long term care facilities ###
### transition from facility to state level ###
####################################################################################################

# Filter out hospital_subtype = long term
# justification: different patient population; not really same sort of "surge" indicator
# also, 0 recorded influenza hospitalizations for long term care in MA
hhs3 = hhs3 %>% filter(hospital_subtype != "Long Term")
# from 13.8k to 11.8k obs


# group by week to estimate state-level influenza, covid admissions
hhs_state = hhs3 %>% 
  group_by(date) %>%
  summarise(flu_weekly_new_MA = sum(flu_weekly_new), 
            covid_weekly_new_pediatric_MA = sum(covid_weekly_new_pediatric),
            covid_weekly_new_adult_MA = sum(covid_weekly_new_adult))
# 195 obs of 4 variables

# how many days between
daysbetween = diff(hhs_state$date) # 7 for all
print(daysbetween)

# create year column
hhs_state$year <- as.numeric(format(hhs_state$date,'%Y'))

# group by week and hospital_subtype to estimate state-level influenza, covid admissions
hhs_state_type = hhs3 %>% # the hhs df with filled in flu, covid data
  group_by(date, hospital_subtype) %>% 
  summarise(flu_weekly_new_MA = sum(flu_weekly_new), 
             covid_weekly_new_pediatric_MA = sum(covid_weekly_new_pediatric),
             covid_weekly_new_adult_MA = sum(covid_weekly_new_adult)) 
# 583 obs of 5 variables

# create year column
hhs_state_type$year <- as.numeric(format(hhs_state_type$date,'%Y'))

####################################################################
### FIGURES FOR FLU, COVID RATES FOR MA ###
###################################################################
...
str(hhs_state)
hhs_state2 = setDT(hhs_state) ## convert to data.table by reference
str(hhs_state2)
cdc3 = setDT(cdc2)
str(cdc3)

hhs_state2[, date1 := date]  ## create a duplicate of 'date1'
cdc3[, date1 := date]  ## create a duplicate of 'date1'

setkey(hhs_state2, date1)    ## set the column to perform the join on
setkey(cdc3, date1)    ## same as above

# join CDC FluSurv to HHS facility data by closest dates rather than exact dates
hhs_cdc = cdc3[hhs_state2, roll=Inf] ## perform rolling join
# 195 obs

summary(hhs_cdc$WEEKLY.RATE.PEDIATRIC) # 0 NAs
summary(hhs_cdc$flu_weekly_new_MA) # 0 NAs

# double check days between weeks
testing = diff(hhs_cdc$date1)
testing
...
hhs_cdc$adult_hosp_flu_cases = (hhs_cdc$flu_weekly_new_MA * (hhs_cdc$WEEKLY.RATE.ADULT/100000)) + 0.00001 # added 0.00001 to avoid issue of zeros for now
summary(hhs_cdc$adult_hosp_flu_cases)
hhs_cdc$pediatric_hosp_flu_cases = hhs_cdc$flu_weekly_new_MA * (hhs_cdc$WEEKLY.RATE.PEDIATRIC/100000) + 0.00001 # added 0.00001 to avoid issue of zeros for now
summary(hhs_cdc$pediatric_hosp_flu_cases)
...
hhs_cdc$adult_hosp_flu_est = hhs_cdc$adult_hosp_flu_scale * hhs_cdc$flu_weekly_new_MA
hhs_cdc$pediatric_hosp_flu_est = hhs_cdc$pediatric_hosp_flu_scale * hhs_cdc$flu_weekly_new_MA
hhs_cdc$total_hosp_flu_est = hhs_cdc$adult_hosp_flu_est + hhs_cdc$pediatric_hosp_flu_est



summary(hhs_cdc$pediatric_hosp_flu_est) # 30 NAs
hist(hhs_cdc$pediatric_hosp_flu_est) # max = 300
hist(hhs_cdc$flu_weekly_new_MA) # max = 800
...
plot(hhs_cdc$date, hhs_cdc$adult_hosp_flu_cases)
plot(hhs_cdc$date, hhs_cdc$adult_hosp_flu_scale)
plot(hhs_cdc$date, hhs_cdc$adult_hosp_flu_est)


plot(hhs_cdc$date, hhs_cdc$total_hosp_flu_est)
plot(hhs_cdc$date, hhs_cdc$flu_weekly_new_MA)


#################################################################
### EXPORT JOINED HHS AND CDC DATA ###
###############################################################
...
Date(df$collection_week, format="%m/%d/%Y"),"%Y")

df <- df%>%
  select("hospital_pk","hospital_name","hospital_subtype","collection_week",
         "state","ccn","city","previous_day_admission_pediatric_covid_confirmed_7_day_sum", "previous_day_admission_influenza_confirmed_7_day_sum",
         "total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum")%>%
  filter(hospital_subtype=='Long Term')

df$previous_day_admission_influenza_confirmed_7_day_sum <- replace(df$previous_day_admission_influenza_confirmed_7_day_sum,df$previous_day_admission_influenza_confirmed_7_day_sum<0,2)
df$total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum <- replace(df$total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum,df$total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum<0,2)
df$previous_day_admission_pediatric_covid_confirmed_7_day_sum <- replace(df$previous_day_admission_pediatric_covid_confirmed_7_day_sum,df$previous_day_admission_pediatric_covid_confirmed_7_day_sum<0,2)

df2 <- df%>%
  group_by(state,Year)%>%
  summarise(sum_flu_covid = sum(total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum),
            sum_flu = sum(previous_day_admission_influenza_confirmed_7_day_sum),
            sum_covid =  sum(previous_day_admission_pediatric_covid_confirmed_7_day_sum))

df2 <- df2%>%
  arrange(desc(sum_flu))

df2%>%
  ggplot(aes(x=state, y= sum_flu))+
  geom_col()+
  labs(title = 'Flu Hospitalizations of Long Term Care Facilities per State')

summary(df2)

#####Counting NA per facility type 
df_summary_NA <- df %>%
  select("state","hospital_subtype", "previous_day_admission_pediatric_covid_confirmed_7_day_sum", "previous_day_admission_influenza_confirmed_7_day_sum", "total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum") %>%
  filter(state=='MA')%>%
  group_by(hospital_subtype) %>%
  summarise(na_count = sum(is.na(previous_day_admission_influenza_confirmed_7_day_sum)))

df_summary_non_NA <- df %>%
  select("state","hospital_subtype", "previous_day_admission_pediatric_covid_confirmed_7_day_sum", "previous_day_admission_influenza_confirmed_7_day_sum", "total_patients_hospitalized_confirmed_influenza_and_covid_7_day_sum") %>%
  filter(state=='MA')%>%
  group_by(hospital_subtype) %>%
  summarise(na_count = sum(!is.na(previous_day_admission_influenza_confirmed_7_day_sum)))
...
df_summary <- df %>%
  select("state","hospital_subtype", "previous_day_admission_influenza_confirmed_7_day_sum") %>%
  filter(state=='MA')%>%
  group_by(hospital_subtype)%>%
  summarise(count=n())
